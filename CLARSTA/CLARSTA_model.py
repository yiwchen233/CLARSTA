...
from .tools import gen_random_directions, eval_objective, check_feasibility

__all__ = ['SampleSet']

class SampleSet(object):
    def __init__(self, p, prand, x0, f0, proj_C, n=None, rel_tol=1e-20):
        if n is None:
            n = len(x0)
        ...
        # The projection list onto feasible set
        self.proj_C = proj_C
    ...
    def initialise_sample_set(self, delta, objfun, nf, maxfun, fmin_true):
        ...
        for i in range(self.p):
            ...
            fea_flag = check_feasibility(self.proj_C, x)

            if fmin_true is not None and f <= fmin_true + self.rel_tol * (self.fbeg - fmin_true) and fea_flag == True:
                self.save_point(x, f)
            ...
            # If x is feasible and has better fval, then update kiter
            if f <= self.fiter() and fea_flag == True:
                self.kiter = i + 1
    ...
    def change_point(self, k, x, f, fea_flag):
        assert 0 <= k <= self.p, "Invalid index %g" % k
        ...
        if self.fvals[k] <= self.fvals[self.kiter] and fea_flag == True:
            self.save_point(x, f)

        return
    
    # Append a point to the sample set
    def append_point(self, x, f, fea_flag):
    ...
    # Remove the k-th point from sample set and update kiter
    def remove_point(self, k):
        ...
        for i in range(self.p + 1):
            x = self.points[i, :]
            if self.fvals[i] <= self.fiter() and check_feasibility(self.proj_C, x):
                self.kiter = i
        ...
        if not diag_only:
            for di_idx in range(self.p):  # index of i-th direction
                for dj_idx in range(di_idx, self.p):  # index of j-th direction
                    ...
                    if f <= fmin_2d and fea_flag == True: 
                        fmin_2d = f
                        self.save_point(x, f)
    ...
    def interpolate_underdetermined_quadratic_model(self, objfun, nf, maxfun, fmin_true):
        c = 0
        g = np.zeros((self.p,))
        H = np.zeros((self.p, self.p))
        exit_info = None
    ...
    def interpolate_quadratic_model(self, objfun, nf, maxfun, fmin_true):
        c = 0
        g = np.zeros((self.p,))
        H = np.zeros((self.p, self.p))
        exit_info = None
    ...
    def interpolate_linear_model_square(self, resfuns, resfun_num):
        c = 0
        g = np.zeros((self.p,))
        H = np.zeros((self.p, self.p))
    ...
    def update_kiter(self, model_type):
        # Record the current kiter and check if there's a better one in current sample set
        kiter_1d = self.kiter
        fiter_1d = self.fiter()
        for i in range(self.p + 1):
            x = self.points[i, :]
            if self.fvals[i] <= fiter_1d and check_feasibility(self.proj_C, x):
                kiter_1d = i
                fiter_1d = self.fvals[i]
        
        # For quadratic models, we have more points (i.e., all xk+di+dj) to check
        if model_type == 'quadratic':
            assert self.fvals_2d is not None, "fvals_2d should not be None after constructing quadratic models, check code!"

            fmin_2d = fiter_1d
            xiter_new_di_idx = None  # potential new xiter's di
            xiter_new_dj_idx = None  # potential new xiter's dj

            # Check if any xk+di+dj is better than current best, and record the coresponding di and dj
            ...
            for di_idx in range(fval_2d_size):  # index of i-th direction
                for dj_idx in range(di_idx, fval_2d_size):  # index of j-th direction
                    ...
                    if f <= fmin_2d and check_feasibility(self.proj_C, self.xiter()+di_idx+dj_idx):
                        xiter_new_di_idx = di_idx
                        xiter_new_dj_idx = dj_idx
                        fmin_2d = f
                        
            # If there's a better xk+di+dj, then we need to update the whole sample set
